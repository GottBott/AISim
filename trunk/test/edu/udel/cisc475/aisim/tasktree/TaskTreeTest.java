...
public class TaskTreeTest {
	...
	public void testGetTotalQuality() {
		...
		assertEquals(20.0D, tree.getTotalQuality(), 0.0001D);
		
		m2.setArrived(false);
		
		assertEquals(0.0D, tree.getTotalQuality(), 0.0001D);
	...
	public void testFindVisibleToAgents() {
		...
		assertFalse(m1.getVisibleToAgents().contains("a1"));
		assertFalse(m1.getVisibleToAgents().contains("a2"));
		
		assertFalse(m2.getVisibleToAgents().contains("a1"));
		assertTrue(m2.getVisibleToAgents().contains("a2"));
		
		assertFalse(t2.getVisibleToAgents().contains("a1"));
		...
		assertFalse(t1.getVisibleToAgents().contains("a1"));
	...
	public void testActivateAllRelationships() {
		...
		e = new DisablesNodeRelationship(t3, m1, "e");
		t3.addRelationship(e);
		t3.setArrived(false);
		assertTrue(m1.isEnabled());
		tree.activateAllRelationships();
	...
	public void testFindArrivals() {
		m1.setArrived(false);
		m1.setArrivalTime(20);
		DisablesNodeRelationship d = new DisablesNodeRelationship(t3, m1, "d");
		t3.addRelationship(d);
		d.setArrivalTime(30);
		d.setArrived(false);
		
		TaskTree tree = new TaskTree(t1);
		
		assertFalse(tree.findArrivals(0L));
		assertFalse(m1.hasArrived());
		assertFalse(d.hasArrived());
		
		assertTrue(tree.findArrivals(20));
		assertTrue(m1.hasArrived());
		assertFalse(d.hasArrived());
		
		assertFalse(tree.findArrivals(25));
		assertTrue(m1.hasArrived());
		assertFalse(d.hasArrived());
		
		assertTrue(tree.findArrivals(30));
		assertTrue(m1.hasArrived());
		assertTrue(d.hasArrived());
	}
}
