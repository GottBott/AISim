...
public class TaskTree {
	...
	private HashMap<String, NodeRelationship> relationships = new HashMap<String, NodeRelationship>();
	/**
	...
	public NodeRelationship getRelationshipByName(String rName){
		return relationships.get(rName);
		...
		for (Node n : nameToNodeMap.values()) {
			if (n instanceof Method) {
				if ((((Method) n).isEnabled() && !((Method) n).isFinished()
						...
						&& n.getDeadline() > tick
						&& n.hasArrived()) && !n.getParent().checkIsFinished()){
					//System.out.println(n.getName());
					//System.out.println(n.getParent().checkIsFinished());
					return false;
				}
			}
			
//			if (n instanceof Method) {
//				if(!n.getParent().checkIsFinished()){
//					return false;
//				}
//
//			}
		...
		for (Node n : nameToNodeMap.values()) {
			if (n.hasArrived()) {
				if ((n instanceof Method && n.isFinished())
					...
					for(int i =0; i < n.relationships.size(); i++){
						relationships.put(n.relationships.get(i).getName(), n.relationships.get(i));
					}
