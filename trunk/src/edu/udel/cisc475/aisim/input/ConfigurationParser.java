...
public class ConfigurationParser {
	...
	private boolean maxTickSet = false;
	...
	public void parse() throws InvalidFileException {
		...
		int maxTick = ConfigurationData.DEFAULT_NUMBER_TICKS;
		...
		try {
			...
			while (sc.hasNextLine()) {
				...
				case "maxTicks":
					if(maxTickSet){
						FileError fe = new FileError(line,
								"Maximum Number Of Ticks Was Already Set");
						errors.add(fe);
						continue;
					}
					try {
						maxTick = Integer.parseInt(tok[1]);
						maxTickSet = true;
					} catch (NumberFormatException e) {
						FileError fe = new FileError(line,
								"maxTickSet Must Be An Integer");
						errors.add(fe);
					}
					break;
