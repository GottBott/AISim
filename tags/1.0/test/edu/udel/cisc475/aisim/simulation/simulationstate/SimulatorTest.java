...
public class SimulatorTest {
	...
	public void testSimFinishesTreeDone() {
		...
		configData = new ConfigurationData(5L, "logs", 4L, PORT);
	...
	// create a tree structure with only one method
		...
		m2.setArrived(false);
		t1.addSubTask(m2);
		EnablesNodeRelationship r = new EnablesNodeRelationship(m1, m2, "e");
		m1.addRelationship(r);
		m2.addRelationship(r);
		TaskTree tree = new TaskTree(t1);
		tree.findVisibleToAgents();
		
		hs = new HashSet<String>();
		hs.add("Agent1");
		configData = new ConfigurationData(5L, "output", 4L, PORT);
		inputData = new InputData(tree, "v5", hs);
		sim = new Simulator(configData, inputData);
